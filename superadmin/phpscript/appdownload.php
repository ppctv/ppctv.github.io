		...
		if ($r['status'] == 'ok' && $r['content']) {
			...
			if ($r) {
				...
				if ($res === TRUE) {
					...
					$zip -> close();
					$r = '应用下载成功，请进入<a class="pk-text-primary pk-hover-underline" href="index.php?c=app&a=superadmin:index&s=template">模板中心</a>或<a class="pk-text-primary pk-hover-underline" href="index.php?c=app&a=superadmin:index&s=app">应用中心</a>进行安装和设置';
				} else {
					$r = '解压失败：' . $res;
				}
			} else {
